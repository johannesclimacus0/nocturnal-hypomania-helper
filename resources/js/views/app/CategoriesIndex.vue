...
const load = async function (): Promise<void> {
    ...
    error.value = ''
    try {
        const response = await http.get<{ data: Entry[] }>('/api/categories')
        entries.value = response.data.data
    } catch {
        error.value = 'Could not load categories. Retry the request.'
    }
    finally {
        loading.value = false
    }
...
const create = async function (): Promise<void> {
    ...
    saved.value = false
    try {
        const response = await http.post<{ data: Entry }>('/api/categories', {
...
<template>
    ...
    <div class="max-w-4xl space-y-5">
        <BaseInput id="categories-search"
                   v-model="search"
                   label="search"
                   type="search"
        ...
        <template v-else>
            ...
            <Panel title="custom entries">
                <EmptyState v-if="!filtered.length" :message="search ? 'no matches' : 'no categories yet'" />
